          ...
          <>
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                ...
                <label className="text-lg font-medium text-purple-800">Your Regular Expression:</label>
              </div>
              <Input
                value={pattern}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="text-lg font-medium text-purple-800">Pattern Breakdown</h2>
                </div>
                <div className="space-y-4">
                  {Object.entries(patternExplanations).map(([category, _]) => {
                    const explanations = analyzePattern(pattern).filter(e => e.category === category);
                    if (explanations.length === 0) return null;
                    
                    return (
                      <div key={category} className="space-y-2">
                        <h3 className="text-sm font-medium text-purple-600 uppercase">{category}</h3>
                        {explanations.map((e, i) => (
                          <div key={i} className="bg-white rounded-lg p-3 shadow-sm border border-purple-100">
                            <code className="text-purple-700 font-mono">{e.token}</code>
                            <p className="text-sm text-gray-600 mt-1">{e.explanation}</p>
                          </div>
                        ))}
                      </div>
                    );
              ...
              <div className="space-y-4">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="text-lg font-medium text-purple-800">Test Laboratory</h2>
                </div>
                  ...
                  {matches.map((result, idx) => (
                    <div key={idx} className="bg-white rounded-lg p-3 shadow-sm border border-purple-100">
                      <div className="font-mono text-sm whitespace-pre-wrap">
                        {result.string.split('').map((char, i) => {
                          ...
                          return (
                            <span
                              key={i}
                              className={match ? 'bg-yellow-200 text-purple-800' : 'text-gray-600'}
                            >
                              {char}
                            </span>
                          );
